  ...
  ({ strapi }) => ({
    async unarchive(ctx) {
      ...
      let entity = await omit(
        await strapi.services["api::order-archive.order-archive"].findOne(id, {
          populate: "*",
        }),
      ...
      let new_entity = await strapi.services["api::order.order"].create({
        data: entity,
      });
      console.log(new_entity);
      if (new_entity === null) return null;

      const delete_entity = await strapi.services[
        "api::order-archive.order-archive"
      ].delete(id);
